...
// ---------------------------------------------------------------------------
// "Suggest a reply" -- POST .../{conversation_id}/suggest-reply. Drafts a
// reply to the conversation's most recent visitor message via the real
// RAG/orchestrator pipeline (same preview_answer Train the Agent's
// "test the bot" uses, new call site). A drafting aid only -- nothing is
// persisted, and there is no live agent-reply-into-conversation endpoint on
// this console to send it through (see transcript-pane.tsx's take-over
// composer note); the agent copies/edits the draft to use through whatever
// channel they actually follow up with.
// ---------------------------------------------------------------------------

/** A cited chunk backing the suggested reply -- mirrors `SuggestedReplySource`
 * (admin_routes.py) exactly. */
export interface SuggestedReplySource {
  docId: string;
  chunkId: string;
  score: number | null;
  matchedBy: string[];
...
interface SuggestedReplySourceBody {
  doc_id: string;
  chunk_id: string;
  score: number | null;
  matched_by: string[];
...
interface SuggestReplyResponseBody {
  conversation_id: string;
  ...
  decision: "answer" | "clarify" | "escalate" | "blocked";
  confidence: number | null;
  sources: SuggestedReplySourceBody[];
}

export type SuggestReplyResult =
  | { status: "ok"; suggestion: SuggestedReply }
  | { status: "error"; message: string; correlationId: string };

function toSuggestedReply(body: SuggestReplyResponseBody): SuggestedReply {
  return {
    conversationId: body.conversation_id,
...
function mapSuggestReplyErrorMessage(error: AdminApiError): string {
  if (error.status === 404 || error.errorCode === "CONVERSATION_NOT_FOUND") {
    return "This conversation could not be found.";
  }
  if (error.errorCode === "NO_VISITOR_MESSAGE") {
    return "There's no visitor message in this conversation to draft a reply to.";
  }
  if (error.status === 403 || error.errorCode === "ROLE_NOT_PERMITTED") {
    return "You do not have permission to draft a reply for this conversation.";
  }
  if (error.status === 401) {
    return "Your session has expired. Please log in again.";
  }
...
/**
 * Draft a reply for the conversation's most recent visitor message. Mirrors
 * `getConversationDetail`'s tenant-scoped-path convention exactly. Never
 * sends `tenant_id`. Never logs the response body (the draft/question text
 * is exactly the PII-bearing content this module otherwise avoids logging).
 */
export async function suggestReply(
  ...
  try {
    const response = await adminApiFetch(
      `${basePath}/${encodeURIComponent(conversationId)}/suggest-reply`,
      { method: "POST" }
    );
    const body = (await response.json()) as SuggestReplyResponseBody;
    return { status: "ok", suggestion: toSuggestedReply(body) };
  } catch (error) {
    if (error instanceof AdminApiError) {
      return {
        status: "error",
        message: mapSuggestReplyErrorMessage(error),
        correlationId: error.correlationId,
      };
    }
    return {
