/**
 * Per-client agent-performance report (platform-admin console). Mirrors
 * `clients/[tenantId]/reports/funnel/page.tsx`'s exact tenant-scoping
 * pattern -- see that file's header comment. Resolves agent names via
 * `listMembersForTenant(tenantId)` (already used by
 * `clients/[tenantId]/leads/page.tsx`), NOT `listMembers()` -- the latter
 * would list the PLATFORM_ADMIN's own (empty) tenant, not this client's.
 */
...
export default async function ClientAgentPerformancePage({ params, searchParams }: PageProps) {
  const { tenantId } = await params;
  const basePath = `/clients/${tenantId}/reports/agent-performance`;
    ...
    <div className="flex flex-1 flex-col gap-[18px] p-[22px] sm:p-[28px]">
      ...
      <div className="flex flex-wrap items-center gap-3.5">
        ...
        <div className="ml-auto flex items-center gap-3">
          <DownloadCsvLink report="agent-performance" query={csvQuery} tenantId={tenantId} />
