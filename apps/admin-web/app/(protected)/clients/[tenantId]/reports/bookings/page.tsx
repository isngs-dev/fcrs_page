/**
 * Per-client bookings report (platform-admin console). Mirrors
 * `clients/[tenantId]/reports/funnel/page.tsx`'s exact tenant-scoping
 * pattern -- see that file's header comment. Bucket toggle (Month included)
 * unchanged from the client-facing page.
 */
...
export default async function ClientBookingsReportPage({ params, searchParams }: PageProps) {
  const { tenantId } = await params;
  const basePath = `/clients/${tenantId}/reports/bookings`;
  ...
  const result = await getBookingsReport({ range, bucket }, tenantId);
    ...
    <div className="flex flex-1 flex-col gap-[18px] p-[22px] sm:p-[28px]">
      ...
      <div className="flex flex-wrap items-center gap-3.5">
        ...
        <div className="ml-auto flex items-center gap-3">
          <DownloadCsvLink report="bookings" query={csvQuery} tenantId={tenantId} />
          <ReportRange
            basePath={basePath}
            currentRange={range}
