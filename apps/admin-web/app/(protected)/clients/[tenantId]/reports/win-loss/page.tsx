/**
 * Per-client win/loss report (platform-admin console). Mirrors
...
export default async function ClientWinLossReportPage({ params, searchParams }: PageProps) {
  const { tenantId } = await params;
  const basePath = `/clients/${tenantId}/reports/win-loss`;
  ...
  const result = await getWinLossReport({ range }, tenantId);
    ...
    <div className="flex flex-1 flex-col gap-[18px] p-[22px] sm:p-[28px]">
      ...
      <div className="flex flex-wrap items-center gap-3.5">
        ...
        <div className="ml-auto flex items-center gap-3">
          <DownloadCsvLink report="win-loss" query={csvQuery} tenantId={tenantId} />
