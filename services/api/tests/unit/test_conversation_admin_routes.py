...
# ---------------------------------------------------------------------------
# POST /admin/conversations/{conversation_id}/suggest-reply
# ---------------------------------------------------------------------------


def _fake_preview_result(
    *, reply: str = "We're open Monday through Friday.", decision: str = "answer",
    confidence: float | None = 0.9,
) -> PreviewResult:
    return PreviewResult(reply=reply, decision=decision, confidence=confidence, sources=[])


async def test_suggest_reply_happy_path_uses_last_visitor_message(app: Any, db: _StubDatabase) -> None:
    """Drafts a reply from the conversation's most recent visitor message --
    not the first, and not a bot message -- via preview_answer."""
    db.seed_conversation(tenant_id=_TENANT_ID, conversation_id="conv-1")
    db.seed_message(
        tenant_id=_TENANT_ID, conversation_id="conv-1", message_id="m1",
        role="user", content="how much does an inspection cost?", created_at=_NOW,
    )
    db.seed_message(
        tenant_id=_TENANT_ID, conversation_id="conv-1", message_id="m2",
        role="bot", content="Let me check on that.", created_at=_NOW,
    )
    db.seed_message(
        tenant_id=_TENANT_ID, conversation_id="conv-1", message_id="m3",
        role="user", content="also, do you serve Georgia?", created_at=_NOW,
    )

    with patch(
        "api.conversation_store.admin_routes.preview_answer",
        AsyncMock(return_value=_fake_preview_result(reply="Yes, we serve Georgia.")),
    ) as mock_preview:
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            token = _token(Role.CLIENT_ADMIN)
            response = await client.post(
                "/admin/conversations/conv-1/suggest-reply", cookies={"access_token": token},
            )

    assert response.status_code == 200
    body = response.json()
    assert body["conversation_id"] == "conv-1"
    assert body["source_message_id"] == "m3"
    assert body["question"] == "also, do you serve Georgia?"
    assert body["reply"] == "Yes, we serve Georgia."
    assert body["decision"] == "answer"
    assert body["confidence"] == 0.9
    assert "tenant_id" not in body
    # preview_answer's own message arg is the LAST visitor message, not the first.
    assert mock_preview.await_args.args[2] == "also, do you serve Georgia?"


async def test_suggest_reply_client_agent_allowed(app: Any, db: _StubDatabase) -> None:
    """Unlike Train the Agent's teach/dismiss (CLIENT_ADMIN-only), suggest-
    reply never touches the knowledge base, so CLIENT_AGENT is allowed --
    matches this file's own list/detail RBAC, not ingestion's convention."""
    db.seed_conversation(tenant_id=_TENANT_ID, conversation_id="conv-1")
    db.seed_message(tenant_id=_TENANT_ID, conversation_id="conv-1", message_id="m1", role="user")

    with patch(
        "api.conversation_store.admin_routes.preview_answer",
        AsyncMock(return_value=_fake_preview_result()),
    ):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            token = _token(Role.CLIENT_AGENT)
            response = await client.post(
...
async def test_suggest_reply_missing_conversation_returns_404(app: Any, db: _StubDatabase) -> None:
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        token = _token(Role.CLIENT_ADMIN)
        response = await client.post(
            "/admin/conversations/does-not-exist/suggest-reply", cookies={"access_token": token},
        )
...
async def test_suggest_reply_cross_tenant_returns_404_no_leak(app: Any, db: _StubDatabase) -> None:
    db.seed_conversation(tenant_id=_TENANT_ID, conversation_id="conv-cross")
    db.seed_message(tenant_id=_TENANT_ID, conversation_id="conv-cross", message_id="m1", role="user")

    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        token = _token(Role.CLIENT_AGENT, tenant_id=_OTHER_TENANT_ID)
        response = await client.post(
            "/admin/conversations/conv-cross/suggest-reply", cookies={"access_token": token},
        )
...
async def test_suggest_reply_no_visitor_message_returns_422(app: Any, db: _StubDatabase) -> None:
    """A conversation with only bot/system messages (or none at all) has
    nothing to draft a reply to -- an honest 422, not a fabricated draft."""
    db.seed_conversation(tenant_id=_TENANT_ID, conversation_id="conv-bot-only")
    db.seed_message(
        tenant_id=_TENANT_ID, conversation_id="conv-bot-only", message_id="m1",
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        token = _token(Role.CLIENT_ADMIN)
        response = await client.post(
            "/admin/conversations/conv-bot-only/suggest-reply", cookies={"access_token": token},
        )

    assert response.status_code == 422
    assert response.json()["error_code"] == "NO_VISITOR_MESSAGE"


async def test_suggest_reply_visitor_forbidden(app: Any, db: _StubDatabase) -> None:
    db.seed_conversation(tenant_id=_TENANT_ID, conversation_id="conv-1")
...
async def test_suggest_reply_no_auth_returns_401(app: Any, db: _StubDatabase) -> None:
    db.seed_conversation(tenant_id=_TENANT_ID, conversation_id="conv-1")

    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        response = await client.post("/admin/conversations/conv-1/suggest-reply")

    assert response.status_code == 401


async def test_suggest_reply_platform_admin_forbidden_on_implicit_route(
    app: Any, db: _StubDatabase,
) -> None:
    db.seed_conversation(tenant_id=_TENANT_ID, conversation_id="conv-1")
...
async def test_suggest_reply_platform_admin_via_tenant_scoped_route_200(
    app: Any, db: _StubDatabase,
) -> None:
    db.seed_conversation(tenant_id=_TENANT_ID, conversation_id="conv-1")
    db.seed_message(tenant_id=_TENANT_ID, conversation_id="conv-1", message_id="m1", role="user")
    ...
    with patch(
        ...
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
            token = _token(Role.PLATFORM_ADMIN, tenant_id=None)
            response = await client.post(
                f"/admin/tenants/{_TENANT_ID}/conversations/conv-1/suggest-reply",
                cookies={"access_token": token},
            )
...
async def test_suggest_reply_never_leaks_tenant_id(app: Any, db: _StubDatabase) -> None:
    db.seed_conversation(tenant_id=_TENANT_ID, conversation_id="conv-1")
    db.seed_message(tenant_id=_TENANT_ID, conversation_id="conv-1", message_id="m1", role="user")
